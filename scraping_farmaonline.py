...
if os.path.exists(ARCHIVO_FALLBACK):
    try:
        ...
        df_fb_cache = df_fb_cache[df_fb_cache['Farmacia'].str.lower().str.contains('farmaonline', na=False)]
...
print("--- INICIANDO SCRAPER FARMAONLINE (DIRECTO A KILLERS) ---")
...
# --- FASE 1: BÚSQUEDA DIRIGIDA POR KILLERS (EAN & NOMBRE) ---
    ...
    try:
        ...
        for k in killers_a_buscar:
            ...
            if not encontrado and ean_target:
                url_target = f"https://www.farmaonline.com/api/catalog_system/pub/products/search?fq=alternateIds_Ean:{ean_target}"
                try:
            ...
            if not encontrado and k.get('sku'):
                url_sku = f"https://www.farmaonline.com/api/catalog_system/pub/products/search?fq=skuId:{k['sku']}"
                try:
            ...
            if not encontrado and nombre_limpio:
                url_nombre = f"https://www.farmaonline.com/api/catalog_system/pub/products/search/{urllib.parse.quote(nombre_limpio)}"
                try:
            ...
            if not encontrado and k.get('nombre_corto'):
                url_corto = f"https://www.farmaonline.com/api/catalog_system/pub/products/search/{urllib.parse.quote(k['nombre_corto'])}"
                try:
            ...
            if not encontrado and k.get('nombre_seguro') and k.get('nombre_seguro') != k.get('nombre_corto') and k.get('nombre_seguro') != '':
                url_seguro = f"https://www.farmaonline.com/api/catalog_system/pub/products/search/{urllib.parse.quote(k['nombre_seguro'])}"
                try:
            ...
            if encontrado and res_item:
                ...
                print(f"     [!] No disponible en API Farmaonline.")
            time.sleep(0.5)
