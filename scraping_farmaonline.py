...
# --- CONFIGURACIÓN ---
ARCHIVO_INPUT = r"C:\Users\b_saravia\Downloads\Listado URL - colecciones Farmaonline.xlsx" 
...
print("--- INICIANDO SCRAPER FARMAONLINE (VIA API VTEX) ---")
...
try:
    ...
    if 'URL' not in df_input.columns:
        df_input = pd.read_excel(ARCHIVO_INPUT, header=None)
        df_input.columns = ['URL', 'GRUPO'] if len(df_input.columns) >= 2 else ['URL']
        if 'GRUPO' not in df_input.columns:
            df_input['GRUPO'] = 'General'
    df_input = df_input.dropna(subset=['URL'])
    df_input['GRUPO'] = df_input['GRUPO'].astype(str)
...
for i, fila in enumerate(lista_input):
    ...
    api_endpoint = url_base.replace("https://www.farmaonline.com/", "https://www.farmaonline.com/api/catalog_system/pub/products/search/")
...
# --- FASE 2: BÜSQUEDA DIRIGIDA POR KILLERS (EAN) ---
    ...
    try:
        ...
        for x in df_killers['EAN'].dropna():
            ...
            if s and s not in eans_ya_encontrados:
                eans_killers.append(s)
        
        print(f"Se identificaron {len(eans_killers)} Killers faltantes.")
        
        for ean in eans_killers:
            print(f"  -> Buscando EAN Killer: {ean}...")
            # NUEVO: Búsqueda exacta por EAN en VTEX usando fq=alternateIds_Ean
            url_target = f"https://www.farmaonline.com/api/catalog_system/pub/products/search?fq=alternateIds_Ean:{ean}"
            try:
                res = session.get(url_target, timeout=10)
                if res.status_code in [200, 206] and res.json():
                    ...
                    if encontrado:
                        ...
                        print(f"     [!] EAN no hallado exacto en resultados.")
                else:
...
if productos_extraidos:
    ...
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Resultados_scraping", f"Output_Farmaonline_VTEX_{ts}.xlsx")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    df_out = df_out.drop_duplicates(subset=['EAN', 'Nombre'])
    df_out.to_excel(path, index=False)
    print(f"\n¡Éxito! Archivo Farmaonline finalizado con {len(df_out)} productos en: {path}")
else:
